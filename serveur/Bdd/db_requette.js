...
async function utilisateur(request, response) {
    try {
        let responseSent = false;
        const sql = 'SELECT SUM(emballage) AS emballage , SUM(pain) AS pain, SUM(alimentaire) AS alimentaire FROM dechet;';
        const totalgeneral = "SELECT FORMAT(SUM(pain + alimentaire + emballage), 3)  AS total_jour FROM dechet JOIN user ON dechet.id_user = user.id_user WHERE DATE(horodatage) = CURDATE() AND user.id_user = (SELECT id_user FROM user WHERE login = 'PARAMETRE');";
        const sql3 = "SELECT DAYNAME(horodatage) AS jour, FORMAT(SUM(pain + alimentaire + emballage),3) AS total_journalier FROM dechet JOIN user ON dechet.id_user = user.id_user WHERE YEARWEEK(horodatage)=YEARWEEK(NOW()) GROUP BY jour AND user.id_user = (SELECT id_user FROM user WHERE login = 'PARAMETRE');"
        const mois = "SELECT FORMAT(SUM(pain + alimentaire + emballage),3)  AS total_mois FROM dechet JOIN user ON dechet.id_user = user.id_user WHERE MONTH(horodatage) = MONTH(CURRENT_DATE()) AND user.id_user = (SELECT id_user FROM user WHERE login = 'PARAMETRE');"
        const annee = "SELECT DAY(horodatage) AS jour_du_mois, FORMAT(SUM(pain + alimentaire + emballage),3) AS total_annee FROM dechet JOIN user ON dechet.id_user = user.id_user  WHERE MONTH(horodatage) = MONTH(CURRENT_DATE()) GROUP BY jour_du_mois ORDER BY jour_du_mois AND user.id_user = (SELECT id_user FROM user WHERE login = 'PARAMETRE');"
        const jourdumois = "SELECT FORMAT(SUM(pain + alimentaire + emballage),3) AS total_dujour_dumois FROM dechet JOIN user ON dechet.id_user = user.id_user WHERE YEAR(horodatage) = YEAR(CURRENT_DATE()) AND user.id_user = (SELECT id_user FROM user WHERE login = 'PARAMETRE');"
        const depuiscreation = "SELECT FORMAT(SUM(pain + alimentaire + emballage),3) AS total_creation FROM dechet JOIN user ON dechet.id_user = user.id_user AND user.id_user = (SELECT id_user FROM user WHERE login = 'PARAMETRE');"
        const result = await db.query(sql);
        const resultatgeneral = await db.query(totalgeneral);
        const resultat_mois = await db.query(mois);
        const result_annee = await db.query(annee);
